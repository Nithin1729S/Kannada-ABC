...
const doodle = {
  ...
  visible: ({ i, fill = 'rgba(0,0,0,1)', stroke }: VisibleProps) => {
    const delay = i * 0.25
    return {
      opacity: 1,
      pathLength: 1,
      ...
      transition: {
        ...
        fill: { delay, duration: stroke ? 0.8 : 0 },
      },
    }
  },
}

export function MotionBoyDoodle() {
  const [fill, mouthFill] = useToken('colors', ['text', 'red.400'])
  const strokeWidth = 2
  return (
    <motion.svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox={`0 0 ${244.75 + strokeWidth * 2} ${262.75 + strokeWidth * 2}`}
      initial="hidden"
      whileInView="visible"
    ...
    >
      ...
      <motion.path
        variants={doodle}
        custom={{ i: 2.25, fill, stroke: true }}
